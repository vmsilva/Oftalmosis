...
if(isset($_SESSION['cd_usuario_log'])|| isset($_SESSION['cd_empresa_usu'])){
    ...
    switch (strtolower($_POST['opr'])){
        case 'confirmasolicitacaodevolucaoprontuario':
            confirmaSolicitacaoDevolucaoProntuario();
            break;
        default:
            break;
    }
...
Function confirmaSolicitacaoDevolucaoProntuario(){
    ...
    if(count($rs)>0){
        ...
        foreach ($G_MEC->recebePost($_POST, 'lista') as $key => $value) {
            ...
            if(SCP_M0002::confirmaSolicitacaoProntuarioAberto()>0){
                ...
                $st_solic_mov_aux = 1;
                if(SCP_M0006::AlterarDevolucao($st_solic_mov_aux)>0){
                    $G_MEC->TransacaoFinaliza();
                    $json['ret']=  'true';
                    $json['mostra'] = 'true';
                    $json['form']=  $form;
                    $json['dados']=  'true';
                    $json['msg'] = 'Prontuário(s) Devolvido com Sucesso!';
                    echo json_encode($json);
                    exit();
                }else{
                    ...
                    $json['mostra'] = 'true';
                    $json['form']=  $form;
                    $json['msg'] = 'Erro: Falha ao Devolver Prontuário! '.$value['nr_prontuario'];
                    echo json_encode($json);
                    exit();
                }
            }else{
