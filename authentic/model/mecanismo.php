...
function __autoload($class_name) {
    ...
    switch ($classe[0]) {
        case 'SCA': //Sistema Controle de Acesso
            ...
            break;
        case 'SGA': //Sistema Gerenciamento Ambulatorial
            require_once BASE.strtolower($classe[0]).'/model/'.$class_name.'.php';
            break;
        ...
        case 'SMG': //Sistema Modulo Gerencial
            require_once BASE.strtolower($classe[0]).'/model/'.$class_name.'.php';
            break;
        case 'SAM': //Sistema atendimento Manual
            require_once BASE.strtolower($classe[0]).'/model/'.$class_name.'.php';
            break;
        case 'SFT': //Sistema Faturamento
            require_once BASE.strtolower($classe[0]).'/model/'.$class_name.'.php';
            break;
        default:
            return;
...
class Mecanismo extends Funcoes{
    ...
    Public Static Function geraMaximoCodigo($codigo, $table, $where = null){
        ...
        if($rsChave[0]['codigo']>0){
            return $rsChave[0]['codigo'] + 1;
        }else{
            return (int)1;
        }
    }

    Public Static Function geraMaximoCodigoAgenda($codigo, $table, $where){
    ...
    Public Static Function geraMaximoCodigoYYYYMM($codigo, $table, $dt_atend, $where = null){
    ...
    Public Static Function geraMaximoCodigoYYYYMMDD($codigo, $table, $dt_atend, $where = null){

        $sql = 'Select coalesce(max('.$codigo.'),0) as codigo';
        $sql.= ' From '.$table;
        $sql.= ' Where substr('.$codigo.',1,8) = '.substr(self::TrataString($dt_atend),1,8);
        if(!is_null($where)){
            $sql.= ' and '.$where;
        }
        ...
        if($rs['codigo'] > 0){
            ...
            return (int)substr(self::TrataString($dt_atend),1,8).'0001';
        } 
    }
...
CLASS GERAL_UPLOAD{
	...
	Public Function Salva(){
            ...
            $dir_sistema = '../../../sistema/'.$this->in_sigla.'/dados/';
            $nm_dir_tmp = $this->nm_dir_dest['tmp_name']['realupload'];
            $nm_arq_ori = $this->nm_dir_dest['name']['realupload'];
            if(file_exists($dir_sistema.$nm_arq_ori)){unlink($dir_sistema.trim($nm_arq_ori));}
            if(!(file_exists($dir_sistema.$nm_arq_ori))){
                ...
                unlink($dir_sistema.trim($nm_arq_ori));
                return false;
            }
	}
}
