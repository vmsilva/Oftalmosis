<?php session_start();
    ...
    function incluir(){
        ...
        $hr_ini_atend = str_replace(':','',$hr_ini_atend);
        $hr_fin_atend = $G_MEC->recebePost($_POST, 'hr_fin_atend');
        $hr_fin_atend = str_replace(':','',$hr_fin_atend);
        ...
        if(trim($hr_ini_atend) >= trim($hr_fin_atend)){
            $json['ret']=  'false';
            $json['mostra'] = 'true';
            $json['form']=  $form;
            $json['msg'] = 'Erro: Hora Inicial não Pode ser Maior que Hora Final!';
            $dados['dados'];
            echo json_encode(array('dados'=>$json));
            exit();
        }
        ...
        if(trim($nr_id_min) == NULL || trim($nr_id_min) == ''){
            ...
            $json['mostra'] = 'true';
            $json['form']=  $form;
            $json['msg'] = 'Erro: Número Minimo Idade não Informado!';
            echo json_encode(array('dados'=>$json));
            exit();
        }
        ...
        if(trim($nr_id_max) == NULL || trim($nr_id_max) == ''){
            ...
            $json['mostra'] = 'true';
            $json['form']=  $form;
            $json['msg'] = 'Erro: Número Máximo Idade não Informado!';
            echo json_encode(array('dados'=>$json));
            exit();
        }
        
        if(trim($nr_id_min) > trim($nr_id_max)){
            $json['ret']=  'false';
            $json['mostra'] = 'true';
            $json['form']=  $form;
            $json['msg'] = 'Erro: Idade Minima não Pode ser Maior  que Idade Maxima!';
            $dados['dados'];
            echo json_encode(array('dados'=>$json));
            exit();
        }
        ...
        SGA_M0012::sethr_fin_atend(null);
        SGA_M0012::sethr_ini_atend($hr_ini_atend);
        $rsHoraInicial = SGA_M0012::validarIntervaloHora();
        if(count($rsHoraInicial)>0){
            $json['ret']=  'false';
            $json['mostra'] = 'true';
            $json['form']=  $form;
            $json['msg'] = 'Erro: Hora Inicial pertence a Intervalo Cadastrado!';
            echo json_encode(array('dados'=>$json));
            exit();
        }
        ...
        SGA_M0012::sethr_ini_atend(null);
        $rsHoraFinal = SGA_M0012::validarIntervaloHora();
        if(count($rsHoraFinal)>0){
            $json['ret']=  'false';
            $json['mostra'] = 'true';
            $json['form']=  $form;
            $json['msg'] = 'Erro: Hora Final pertence a Intervalo Cadastrado!';
            echo json_encode(array('dados'=>$json));
            exit();
        }
        
        //Intervalo Hora Inicial e Hora Final
        SGA_M0012::sethr_fin_atend($hr_fin_atend);
        SGA_M0012::sethr_ini_atend($hr_ini_atend);
        $rsIntervaloHora = SGA_M0012::validarIntervaloHora();
        if(count($rsIntervaloHora)>0){
            $json['ret']=  'false';
            $json['mostra'] = 'true';
            $json['form']=  $form;
            $json['msg'] = 'Erro: Hora Inical ou Hora Final pertence a Intervalo Cadastrado!';
            echo json_encode(array('dados'=>$json));
            exit();
        }
        ...
        SGA_M0012::setdt_cad_grade($G_MEC->DataHoje());
        ...
        if($rs){
            ...
            $json['mostra'] = 'true';
            $json['form']=  $form;
            $json['msg'] = 'Sucesso: Grade Cadastrada com Sucesso!';
            $dados['dados'];
            echo json_encode(array('dados'=>$json));
            exit();
            ...
            $json['mostra'] = 'true';
            $json['form']=  $form;
            $json['msg'] = 'Erro: Grade não Cadastrada!';
            $dados['dados'];
            echo json_encode(array('dados'=>$json));
            exit();
        }
