...
if(isset($_SESSION['cd_usuario_log'])|| isset($_SESSION['cd_empresa_usu'])){
    ...
    $cd_usu = $_SESSION['cd_usuario_log'];
    $cd_empresa = $_SESSION['cd_empresa_usu'];
    $cd_ex_laudo = $G_MEC->recebePost($_GET, 'cd_ex_laudo');
    
    
    if(trim($cd_ex_laudo) == '' || trim($cd_ex_laudo) == NULL){
        $json['ret']=  'false';
        $json['mostra'] = 'true';
        $json['form']=  $form;
        $json['msg'] = 'Erro: Não e Possivel Localizar Laudo, Código não Informado!';
    ...
    SGA_M0008::setcd_ex_laudo($cd_ex_laudo);
    SGA_M0008::setcd_usu($cd_usu);
    $rs = SGA_M0008::buscaLaudo();
    ...
    if(count($rs)>0){
        ...
        $PDF->SetTitle('Laudo Procedimento');
        ...
        $PDF->Cell(190, 3, iconv('utf-8', 'iso-8859-1', 'L A U D O - P R O C E D I M E N T O'), 0, 0, 'C', '', '');
        ...
        $PDF->SetXY(154, 10);$PDF->Cell(2, 9, 'Sexo:', 0, 0, 'L', '', '');
        $PDF->SetXY(175, 10);$PDF->Cell(2, 9, 'D.N.:', 0, 0, 'L', '', '');
        $PDF->SetXY(65, 10);$PDF->Cell(2, 19, iconv('utf-8', 'iso-8859-1', 'Mãe:'), 0, 0, 'L', '', '');
        $PDF->SetXY(155, 10);$PDF->Cell(2, 19, 'CNS:', 0, 0, 'L', '', '');   
        $PDF->SetXY(65, 10);$PDF->Cell(2, 39, iconv('utf-8', 'iso-8859-1', 'Prontuário:'), 0, 0, 'L', '', '');
        $PDF->SetXY(65, 10);$PDF->Cell(2, 59, iconv('utf-8', 'iso-8859-1', 'Procedimento'), 0, 0, 'L', '', '');
        ...
        $PDF->SetXY(164, 9);$PDF->Cell(7, 10, iconv('utf-8', 'iso-8859-1',substr($rs[0]['tp_sexo_pac'],0,40)), 0, 0, 'L', '', '');
        $PDF->SetXY(184, 9);$PDF->Cell(7, 10, iconv('utf-8', 'iso-8859-1',substr(Mecanismo::FormataDatacomBarra($rs[0]['dt_nasc_pac']),0,40)), 0, 0, 'L', '', '');
        $PDF->SetXY(74, 14);$PDF->Cell(7, 10, iconv('utf-8', 'iso-8859-1',substr($rs[0]['nm_mae_pac'],0,45)), 0, 0, 'L', '', '');
        $PDF->SetXY(166, 14);$PDF->Cell(7, 10, iconv('utf-8', 'iso-8859-1',substr($rs[0]['nr_cns_pac'],0,40)), 0, 0, 'L', '', '');        
        $PDF->SetXY(85, 25);$PDF->Cell(7, 10, iconv('utf-8', 'iso-8859-1',substr($rs[0]['nr_prontuario'],0,40)), 0, 0, 'L', '', '');
        $PDF->SetXY(90, 35);$PDF->Cell(7, 10, iconv('utf-8', 'iso-8859-1',substr($rs[0]['nm_procd_medc'],0,40)), 0, 0, 'L', '', '');
                
        $PDF->SetTextColor(100,149,237);
        $PDF->SetFont('Arial', '', 13);
        $PDF->SetXY(10, 60);$PDF->Cell(2, 9, 'OLHO DIREITO:', 0, 0, 'L', '', '');
        $PDF->SetXY(10, 61);$PDF->Cell(2, 9, '____________', 0, 0, 'L', '', '');
        $PDF->SetXY(10, 100);$PDF->Cell(2, 9, 'OLHO ESQUERDO:', 0, 0, 'L', '', '');
        $PDF->SetXY(10, 101);$PDF->Cell(2, 9, '______________', 0, 0, 'L', '', '');
        $PDF->SetXY(10, 140);$PDF->Cell(2, 9, 'CONCLUSAO:', 0, 0, 'L', '', '');
        $PDF->SetXY(10, 141);$PDF->Cell(2, 9, '____________', 0, 0, 'L', '', '');
        
        $PDF->SetTextColor(0,0,0);
        $PDF->SetFont('Arial', '', 10);       
        $PDF->SetXY(30, 70);$PDF->MultiCell(0,5,iconv('utf-8', 'iso-8859-1',$rs[0]['ds_olho_dir']),0,1);        
        $PDF->SetXY(30, 110);$PDF->MultiCell(0,5,iconv('utf-8', 'iso-8859-1',$rs[0]['ds_olho_esq']),0,1);
        $PDF->SetXY(30, 150);$PDF->MultiCell(0,5,iconv('utf-8', 'iso-8859-1',$rs[0]['ds_conclusao']),0,1);
        
        $PDF->SetTextColor(0,0,0);
        $PDF->SetFont('Arial', '', 10);
        $PDF->SetXY(20, 260);$PDF->Cell(7, 10, iconv('utf-8', 'iso-8859-1',substr($G_MEC->FormataDatacomBarra(Mecanismo::DataHoje()),0,40)), 0, 0, 'L', '', '');
        $PDF->SetFont('Arial', '', 8);
        $PDF->SetXY(150, 260);$PDF->Cell(7, 10, iconv('utf-8', 'iso-8859-1',substr('Medico: '.$rs[0]['nm_usuario'],0,40)), 0, 0, 'L', '', '');
        $PDF->SetXY(160, 264);$PDF->Cell(7, 10, iconv('utf-8', 'iso-8859-1','CRM: '.$rs['0']['nr_conselho']), 0, 0, 'L', '', '');
...
//        
        ...
        $json['mostra'] = 'true';
        $json['form']=  $form;
        $json['msg'] = 'Erro: Laudo não Encontrado!';
        echo json_encode(array('dados'=>$json));
        exit();
    }
...
//    $PDF->SetXY(65, 4);
//    $PDF->Cell(130, 5, iconv('utf-8', 'iso-8859-1', ''), 0, 0, 'C', '', '');
//    $PDF->SetXY(65, 4);
//    $PDF->Cell(130, 5, iconv('utf-8', 'iso-8859-1', 'RETINA'), 0, 0, 'C', '', '');
//    $PDF->SetXY(65, 10);
...
//    $nameUsuario = '-'.substr('Victor',0,40);
//    $PDF->Footer('B',$nameUsuario);
//    $pdf -> SetXY(10, 40);
    //$PDF->SetXY(0,10);
    
    
    //exit("boceta Cabeluda");
    
    
    
//    if($G_MEC->recebePost($_GET, 'cd_ex_laudo') === NULL || $G_MEC->recebePost($_GET, 'cd_ex_laudo')=== ''){
